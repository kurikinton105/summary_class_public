...
import numpy as np
from CreateAdditionalExplanation import Extract_ImportantWords
...
def bag_of_words_sum(input_text,compression,number_of_sentence): #input_text：入力テキスト,compression：圧縮率,number_of_sentence：文字数制限
    ...
    #専門用語があったら入れ替える
    #output_kyaku = output
    wordlist = Extract_ImportantWords(input_text_word)
    """
    #print(wordlist)
    for i in range(len(wordlist)):
      for j in range(len(output_kyaku)):
        if wordlist[i] in output_kyaku[j]:
          if wordlist[i] !="":
            output_kyaku[j] = output_kyaku[j].replace(str(wordlist[i]),"<a href="+"https://www.google.co.jp/search?q="+str(wordlist[i])+"> "+str(wordlist[i])+" </a>")
    """
    # リストから文章に変更する。
    output_text = ""
...
##以下tomsoyaN追加分
def make_summary(input_text,compression,number_of_sentence): #input_text：入力テキスト,compression：圧縮率,number_of_sentence：文字数制限
